...
TSharedRef<SWidget> URoadToolbarWidget::RebuildWidget()
{
    if (WidgetTree && !WidgetTree->RootWidget)
    {
        ...
        ToggleLabel->SetColorAndOpacity(FSlateColor(FLinearColor::Black));
        ToggleButton->AddChild(ToggleLabel);

        UCanvasPanelSlot* ToggleSlot = Canvas->AddChildToCanvas(ToggleButton);
        ...
        DrawButtonLabel->SetColorAndOpacity(FSlateColor(FLinearColor::Black));
        DrawButton->AddChild(DrawButtonLabel);
        UVerticalBoxSlot* DrawSlot = Box->AddChildToVerticalBox(DrawButton);
        DrawSlot->SetPadding(FMargin(0.0f, 0.0f, 0.0f, 6.0f));
        DrawSlot->SetHorizontalAlignment(HAlign_Fill);
...
void URoadToolbarWidget::AddRow(UVerticalBox* Parent, const FText& Label, UWidget* Input)
{
